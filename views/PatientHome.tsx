...
import { User, WalkSession, ExerciseSessionLog, ExerciseVideo } from '../types';
...
export const PatientHome: React.FC<Props> = ({ user, setView }) => {
  ...
  const distanceToday = Math.floor(stepsToday * 0.7); 
  ...
  const exerciseMinutesToday = todaysExercises.reduce((acc, log) => {
      const vid = allVideos.find(v => v.id === log.video_id);
    ...
    <div className="space-y-6 pt-2">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-md border-l-8 border-blue-600">
        ...
        <div className="w-full bg-gray-200 h-3 rounded-full overflow-hidden mb-2">
             <div className="bg-blue-600 h-full transition-all duration-1000" style={{ width: `${Math.min((totalMinutes/timeGoal)*100, 100)}%` }}></div>
        </div>
      ...
      <div className="card-metric">
        ...
        <progress className="w-full h-4 rounded-full overflow-hidden [&::-webkit-progress-bar]:bg-gray-200 [&::-webkit-progress-value]:bg-green-500 [&::-moz-progress-bar]:bg-green-500" value={stepsToday} max={stepGoal}></progress>
      </div>
      ...
      {completedVideos.length > 0 && (
          <div className="bg-green-50 rounded-2xl p-5 border border-green-100 shadow-sm animate-fade-in">
              <h3 className="font-bold text-green-800 text-lg mb-3 flex items-center gap-2"><span>✅</span> Ejercicios Completados Hoy</h3>
      ...
      <button id="btn-iniciar-caminata" className="btn-gigante verde mt-4" onClick={() => setView('tracker')}>▶️ INICIAR CAMINATA</button>

      <div className="space-y-3 pt-2 pb-8">
        <button className="btn-secundario flex items-center justify-center gap-2" onClick={() => setView('exercises')}><span>📹</span> Ver Ejercicios</button>
        <button className="btn-secundario flex items-center justify-center gap-2" onClick={() => setView('nutrition')}><span>🥗</span> Consejos de Comida</button>
      </div>
    </div>
  );
};